...
while ($e = $db->fetch_assoc($result)) {
?>
$db->query('INSERT INTO '.$db->prefix.'glossary_items (src,dst,user_id,username,ctime,mtime,description) VALUES(\''.$db->escape('<?php echo addslashes($e['term_en'])?>').'\',\''.$db->escape('<?php echo addslashes($e['term_vi'])?>').'\',2,\'admin\','$now.','.$now.',\''.$db->escape('<?php echo addslashes($e['note'])?>').'\')');
<?php
}
?>
